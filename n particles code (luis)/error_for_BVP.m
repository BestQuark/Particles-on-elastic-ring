function G = error_for_BVP(config,L,n,Req,Vo,params)
...
Forces = forces(config,n, Req,Vo,params).F;


%first bc
G(1) = end_rod.x1(end) ;
G(2) = end_rod.x2(end) ;
G(3) = wrapToPi(end_rod.x3(end)) ;
...
for i=1:(n-1)
   ...
   G(6+3*(i-1)) = wrapToPi(initial_rod.x3(i+1) - end_rod.x3(i));
end
