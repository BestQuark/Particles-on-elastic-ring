...
for i=1:n-1
    for j=1:n-1 
        dG_x1(3*i+1:3*i+3,6*(j-1)+1: 6*j-3) = -eye(3,3)
    end
end
...
for i = 1:n-1
    for j=1:n-1 
        dG_x1(3*(n-1)+3*i+1:3*(n-1)+3*i+3,4+6*(j-1): 6*j) = -eye(3,3)
    end

%for the last func
for i=1:n-1
   for j=1:n-1
       dG_x1(6*(n-1)+3+i:6*(n-1)+3+i,4+6*(j-1): 6*j) = [0 -cos(x0_3(i+1)) -sin(x0_3(i+1))];
   end
end
